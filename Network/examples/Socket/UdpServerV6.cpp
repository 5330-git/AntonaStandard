...
/*
本程序 sock1 占用::1:9000 sock2 ::1:9001
目标通信程序 sock1 占用::1:10000 sock2 占用::1:10001
...
int main(){
    // 绑定到本地任意地址的9000端口
    Network::UDPSocket sock9000(CPS::SocketAddress::anyAddress(9000,CPS::AddressFamily::ipv6));
    
    // 绑定到9001端口 
    Network::UDPSocket sock9001(CPS::SocketAddress::anyAddress(9001,CPS::AddressFamily::ipv6));
    ...
    // 对需要发送数据的通道设置目标地址
    channel_9000_10001.setAddress(CPS::SocketAddress::loopBackAddress(10001,CPS::AddressFamily::ipv6));
    channel_9001_10000.setAddress(CPS::SocketAddress::loopBackAddress(10000,CPS::AddressFamily::ipv6));
    ...
    std::ostream(&channel_9000_10001.getOutBuffer())<<msg_9000_10000<<"[::1:9000] ";
    ...
    std::ostream(&channel_9001_10000.getOutBuffer())<<msg_9001_10001<<"[::1:9001] ";
