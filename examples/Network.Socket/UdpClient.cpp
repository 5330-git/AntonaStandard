...
/*
本程序 sock1 占用127.0.0.1:10000 sock2 127.0.0.1:10001
目标通信程序 sock1 占用127.0.0.1:9000 sock2 占用127.0.0.1:9001
...
int main(){
    // 绑定到本地任意地址的10000端口
    Network::UDPSocket sock10000(CPS::SocketAddress::anyAddress(10000,CPS::AddressFamily::ipv4));
    
    // 绑定到10001端口 
    Network::UDPSocket sock10001(CPS::SocketAddress::anyAddress(10001,CPS::AddressFamily::ipv4));
    ...
    // 对需要发送数据的通道设置目标地址
    channel_10000_9000.setAddress(CPS::SocketAddress::loopBackAddress(9000,CPS::AddressFamily::ipv4));
    channel_10001_9001.setAddress(CPS::SocketAddress::loopBackAddress(9001,CPS::AddressFamily::ipv4));

    // 从sock10000的channel_10000_9000 通道开始发送数据
    std::ostream(&channel_10000_9000.getOutBuffer())<<"[127.0.0.1:10000] ";
    ...
    std::ostream(&channel_10001_9001.getOutBuffer())<<msg_10001_9000<<"[127.0.0.1:10001] ";
